...
/*const today = new Date();//오늘 날짜//내 컴퓨터 로컬을 기준으로 today에 Date 객체를 넣어줌
...
function buildCalendar() {//현재 달 달력 만들기
    ...
    //테이블에 새로운 열 삽입//즉, 초기화
    const cnt = 0;// count, 셀의 갯수를 세어주는 역할
    // 1일이 시작되는 칸을 맞추어 줌
...
const nextDates = [];//날짜들 담아줄 배열 만들기

if (PLDay !== 6) {
    for (let i = 0; i < PLDay + 1; i++) {
      prevDates.unshift(PLDate - i);//1씩 줄어든 걸 prevDates앞에 계속 채워넣음
    }//토요일이면 다음 월 들여쓸 필요x
  }
  
  for (let i = 1; i < 7 - TLDay; i++) {
    nextDates.push(i);//다음달 (이번달 남은 칸 파악)
  }

const dates = prevDates.concat(thisDates, nextDates);//세 배열 합침
